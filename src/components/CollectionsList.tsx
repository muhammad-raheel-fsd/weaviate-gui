...
export function CollectionsList({
  ...
  const handleExportClick = async (collectionName: string) => {
    try {
      ...
      const response = await fetch(`/api/export/${collectionName}?${params}`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.details || error.error || "Failed to delete collection"
        );
      }
            ...
            <div className="flex justify-between items-center space-x-4">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-2">
                  ...
                  {collection.count === 1 ? "object" : "objects"}
                </p>
              </div>
                 ...
                 <button
                   onClick={() => handleExportClick(collection.name)}
            ...
            <div className="mt-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="text-xs text-gray-500">
                  Vector enabled:{" "}
                  {collection.properties.some((p) => p.name === "vector")
                    ? "✓"
                    : "✗"}
                </div>
              </div>
                  ...
                  (prop: {
                    ...
                    <span
                      ...
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium hover:cursor-help ${
                        prop.dataType && prop.dataType[0] === "text"
                          ? "bg-green-100 text-green-800"
                          : prop.dataType && prop.dataType[0] === "int"
                          ? "bg-yellow-100 text-yellow-800"
                          : prop.dataType && prop.dataType[0] === "date"
                          ? "bg-blue-100 text-blue-800"
                          : prop.dataType && prop.dataType[0] === "boolean"
                          ? "bg-purple-100 text-purple-800"
                          : prop.dataType && prop.dataType[0] === "uuid"
                          ? "bg-indigo-100 text-indigo-800"
                          : prop.dataType && prop.dataType.includes("[]")
                          ? "bg-orange-100 text-orange-800"
                          : prop.dataType && prop.dataType[0] === "blob"
                      ...
                      title={`${
                        ...
                        prop.dataType ? prop.dataType.join(", ") : "unknown"
                      }`}
                    >
                      {prop.name}
                      {prop.dataType &&
                        prop.dataType.some((t) => t.includes("[]")) && (
